...
const input = () => ({ fairProbability: 0.7, quote: quote(), config: live, secondsRemaining: 120, tradeSizeSol: 0.01, aggregateExposureSol: 0, openPositions: 0, dataFresh: true });
...
describe("entry decisions", () => {
  ...
  it("edge_impact_time_size_position_and_aggregate_caps_each_block_entry", () => {
    const cases = [
      ...
      { expected: "size", change: { tradeSizeSol: 0.011 } },
      { expected: "positions", change: { openPositions: 3 } },
      { expected: "aggregateExposure", change: { aggregateExposureSol: 0.05 } },
