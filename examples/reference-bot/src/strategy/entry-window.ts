...
export const polymarketEntryWindow = ({ mode, market, quote, now, earlyWindowSeconds, lateWindowSeconds, submissionBufferSeconds }: {
  mode: PolymarketTimingMode; market: PilotMarket; quote: ExecutableQuote; now: number;
  earlyWindowSeconds: number; lateWindowSeconds: number; submissionBufferSeconds: number;
}): EntryWindowDecision => {
  ...
  const closesAt = mode === "polymarket_early"
    ...
    : quote.closingProtection.closingStartsAt - submissionBufferSeconds;
