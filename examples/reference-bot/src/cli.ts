...
const runPreFeeRevalidationFixtureSmoke = async () => {
  ...
  try {
    ...
    const price = (askBps: number) => ({ tokenId: "token", bidBps: askBps - 100, askBps, spreadBps: 100, observedAtMs: Date.now() });
    const buyQuote = (side: "yes" | "no"): ExecutableQuote => ({ ...sampleQuote, quoteId: `pre-fee-${side}`, side, amount: "100", grossAmount: "100", economics: { ...sampleQuote.economics, grossAmount: "100", projectedWinningPayout: "200" }, closingProtection: { ...sampleQuote.closingProtection, closingStartsAt: now + 15, hardLockTs: now + 30 } });
